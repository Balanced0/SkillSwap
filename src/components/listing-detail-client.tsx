...
export function ListingDetailClient({ id }: { id: string }) {
  ...
  return <div className="shell page-section skill-detail">
    ...
    <div className="detail-grid">
      <section className="detail-main"><p className="eyebrow rust">{listing.category} · {listing.level}</p><h1>{listing.title}</h1><p className="detail-description">{listing.description}</p><div className="tag-row">{listing.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><Link href={`/members/${listing.teacher._id}`} className="teacher-summary flex items-center gap-4 p-4 border border-beige rounded-lg hover:opacity-85 text-inherit" style={{ display: "flex", textDecoration: "none", color: "inherit" }}><Avatar member={listing.teacher} size="lg" /><div><p className="eyebrow">YOUR TEACHER</p><h2>{listing.teacher.name}</h2><p>{listing.teacher.location || "SkillSwap member"} · {listing.teacher.sessionsCompleted ? `${listing.teacher.sessionsCompleted} completed swaps` : "New member"}</p></div></Link><section className="detail-section"><h2>Availability</h2>{listing.availability.length ? <div className="availability-list">{listing.availability.map((slot) => <div key={slot.day}><Calendar /> <strong>{slot.day}</strong><span>{slot.timeSlots.join(" · ")}</span></div>)}</div> : <p className="muted-copy">Ask the teacher to find a time that works for both of you.</p>}</section></section>
